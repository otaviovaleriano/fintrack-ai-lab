...
const Dashboard = () => {
  ...
  const handlePDFExport = () => {
    ...
    autoTable(doc, {
      ...
      body: [
        ...
        ["Saving Goal", goal ? "$" + goal.amount.toFixed(2) : "Not Set"],
        ...spentByCategory.map((item) => [item.name, "$" + item.amount]),
      ],
    });
    ...
    autoTable(doc, {
      ...
      body: incomeSources.map((item) => [item.name, "$" + item.amount]),
    });
  ...
  const handleExcelExport = () => {
    const expenseData = spentByCategory.map((item) => ({
      Category: item.name,
      Amount: item.amount,
    }));

    const incomeData = incomeSources.map((item) => ({
      Source: item.name,
      Amount: item.amount,
    }));
    ...
    saveAs(blob, "FinTrack_Report.xlsx");
  };
  ...
  return (
    <div className="p-6 max-w-6xl mx-auto space-y-6">
      ...
      <div className="flex items-center justify-between mt-6 relative">
        ...
        {/* Tooltip */}
        <div
          className="ml-2 text-gray-500 cursor-pointer w-5 h-5"
